...
extension Reactive where Base: NSManagedObjectContext {
    ...
    /**
     Executes a fetch request and returns the fetched objects as an `Observable` array of
     `NSManagedObjectContext`
     - parameter fetchRequest: an instance of `NSFetchRequest` to describe the search criteria used
     to retrieve data from a persistent store.
     - parameter sectionNameKeyPath: the key path on the fetched objects used to determine the
     section they belong to: defaults to `nil`
     - parameter cacheName: the name of the file used to cache section information; default to `nil`
     - return: An `Observable` array of `NSManagedObjects` objects that can be found a table
     view.
     */
